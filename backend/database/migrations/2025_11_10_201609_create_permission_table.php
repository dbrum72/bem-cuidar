...
class CreatePermissionTable extends Migration {
    ...
    public function up() {
        ...
        Schema::create($tableNames['roles'], function (Blueprint $table) use ($teams) {
            $table->bigIncrements('id');
            if ($teams || config('permission.team_foreign_key')) {
                $table->unsignedBigInteger(config('permission.team_foreign_key'))->nullable();
                $table->index(config('permission.team_foreign_key'), 'roles_team_foreign_key_index');
            }
            $table->string('name');
            $table->string('guard_name');
            $table->timestamps();
        ...
        Schema::create($tableNames['model_has_permissions'], function (Blueprint $table) use ($tableNames, $columnNames, $teams) {
            ...
            if ($teams || config('permission.team_foreign_key')) {
                $table->unsignedBigInteger(config('permission.team_foreign_key'))->nullable();
                $table->index(config('permission.team_foreign_key'), 'model_has_permissions_team_foreign_key_index');
            }
        ...
        Schema::create($tableNames['model_has_roles'], function (Blueprint $table) use ($tableNames, $columnNames, $teams) {
            ...
            if ($teams || config('permission.team_foreign_key')) {
                $table->unsignedBigInteger(config('permission.team_foreign_key'))->nullable();
                $table->index(config('permission.team_foreign_key'), 'model_has_roles_team_foreign_key_index');
            }
